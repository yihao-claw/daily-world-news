...
SCRAPE_SH = SCRIPT_DIR / "x-scrape.sh"
CHECK_PY = SCRIPT_DIR / "x-check-new.py"
STATE_PATH = SCRIPT_DIR / "x-monitor-state.json"

# Brightdata token — must be set via env var BRIGHTDATA_API_TOKEN
...
def main():
    ...
    if not token:
        print("ERROR: BRIGHTDATA_API_TOKEN env var not set", file=sys.stderr)
        sys.exit(1)
